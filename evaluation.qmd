...
line_crossings = st_crossings(bike_lines, car_lines) |>
  remove_rownames() |>
  st_as_tibble() |>
  select(id, id.1) |>
  rename(track_id_bike = id, track_id_car = id.1)
```

```{r}
crossing_interactions = line_crossings |>
  st_filter(focus)
```
...
ggplot() +
  ...
  theme(axis.ticks = element_blank(), axis.text = element_blank(), panel.background = element_blank()) +
  ggtitle("Trajectory-based interaction points", subtitle = "Approach II: Post-encroachment times")
```
...
Create a gridded representation of the focus area with 1x1 meter cells, calculate the trajectory-based interaction point density in each cell, and assign each grid cell a network-based interaction zone. Then, compare the density of trajectory-based interaction points inside the network-based interaction zones to the density of trajectory-based interaction points outside of the network-based interaction zones.

We see here that the density inside the zones is higher on average (note that the area outside of zones is much larger), but that many cells with high interaction point densities are located outside of any interaction zone.
...
ggplot(gridpoints) +
  ...
  ggtitle("Interaction point densities inside and outside of interaction zones")
```
